<?php
// 2D Number UI (00-99) for 4:30PM (မြန်မာစံတော်ချိန်), mobile UI, brake/limit control, balance in table, R button = reverse selection + balance refresh
...
$closedInfo = getSessionClosedInfo('Asia/Yangon', '16:30:00', '16:00:00'); // 4:30PM session, closed at 4:00PM
...
$stmt_current->execute([':type'=>'2D-1630', ':bet_date' => $bet_date]);
        ...
        try {
            ...
            foreach ($selected_numbers as $number) {
                ...
                if ($brake_limit != -1) {
                    ...
                    $stmt_num_total->execute([':type'=> '2D-1630', ':bet_date' => $bet_date, ':number' => $number]);
            ...
            foreach ($selected_numbers as $number) {
                $insert_stmt->execute([':user_id' => $current_user, ':bet_type' => '2D-1630', ':number' => $number, ':amount' => $bet_amount, ':bet_date' => $bet_date]);
            }
...
<body>
    ...
    <div class="container">
        <div class="time-badge">မြန်မာစံတော်ချိန် - ညနေ ၄ နာရီ ၃၀ မိနစ် (4:30PM)<?= htmlspecialchars($display_date_info) ?></div>
...
window.addEventListener('load', function() {
    sessionStorage.removeItem('selected2d_1630');
    selected = {};
...
let selected = JSON.parse(sessionStorage.getItem('selected2d_1630') || '{}');
...
numbersGrid.addEventListener('click', function(e) {
    ...
    sessionStorage.setItem('selected2d_1630', JSON.stringify(selected));
    updateGridSelections();
});
