...
function showDynamicWallets(data) {
  ...
  setKpayAdminPhone();
  // Wave Dai phone set from admin_phones (no API call)
  setWaveAdminPhone();
}
...
function setWaveAdminPhone() {
  if (window.lastWalletAdminPhones && window.lastWalletAdminPhones.wave && document.getElementById('wm-dai-phone')) {
    document.getElementById('wm-dai-phone').textContent = window.lastWalletAdminPhones.wave;
  }
}
...
if (kpayDepositForm) {
  kpayDepositForm.onsubmit = function(e) {
    if (!e) return;
    e.preventDefault();
    const amount = document.getElementById('kpay-deposit-amount').value;
    const txid = document.getElementById('kpay-deposit-txid').value;
    const screenshotInput = document.getElementById('kpay-deposit-screenshot');
    const screenshot = screenshotInput ? screenshotInput.files[0] : null;

    const phone = getPhone();
    if(!phone) {
      showError('kpay-deposit-error', 'အကောင့်ဝင်ပါ။');
      return;
    }
    if (!amount || amount < 1000) {
      showError('kpay-deposit-error', 'ငွေသွင်းပမာဏ မှန်ကန်စွာ ထည့်ပါ။');
      return;
    }
    if (!screenshot && (!txid || txid.length < 6)) {
      showError('kpay-deposit-error', 'Screenshot သို့မဟုတ် လုပ်ငန်းစဉ်နောက် ၆လုံး တစ်ခုခု တင်ပါ။');
      return;
    }
    ...
    const succ = document.getElementById('kpay-deposit-success');
    const err = document.getElementById('kpay-deposit-error');
    ...
    fetch('https://amazemm.xyz/api/wallet_api.php', {
      ...
      .then(data => {
        if (data.success) {
          ...
          if (err) err.style.display = 'none';
          const form = document.getElementById('kpay-deposit-form');
          if (form) form.reset();
          loadWalletData();
          if (typeof loadWalletTransactionHistory === "function") loadWalletTransactionHistory();
        } else {
